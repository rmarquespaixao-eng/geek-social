...
export class FriendsService {
  ...
  async removeFriend(userId: string, friendId: string): Promise<void> {
    const friendship = await this.repo.findExistingRelation(userId, friendId)
    if (!friendship || friendship.status !== 'accepted') throw new FriendsError('NOT_FOUND')
    await this.repo.deleteById(friendship.id)
  }
