...
| Adversário | Capacidade | Mitigação |
|---|---|---|
| Operador com acesso ao DB | Lê todas as tabelas | Mensagens são ciphertext; identity privada só com PIN encriptado (PBKDF2 600k) |
| Atacante MITM no transporte | Vê tráfego TLS terminado | Mesmo se quebrar TLS, ainda é só ciphertext; identity TOFU pega troca de chave |
| Adversário com computador quântico (futuro) | Quebra Curve25519 | Kyber1024 no PQXDH preserva o segredo |
| Atacante rouba JWT vazado | Pode falar pelo user no chat | Mas **não** lê histórico — não tem chave privada do IDB do dispositivo legítimo |
| Roubo do dispositivo desbloqueado | Acesso total ao IDB | Fora de escopo (mesmo problema do Signal/WhatsApp); roadmap: lock screen no app |
| Roubo do dispositivo bloqueado | OS bloqueia acesso ao IDB | OK |
| Compromise do servidor de push | Sabe o `messageId` | Não decifra: payload é metadata-only |
| User esquece PIN | Backup inacessível | Restore impossível — perde histórico desse dispositivo, mas não as mensagens em outros peers; precisa recriar identity (peers vão precisar verificar nova safety number) |
