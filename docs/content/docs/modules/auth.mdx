...
<Mermaid chart={`sequenceDiagram
  ...
  FE->>API: POST /auth/register {email, password, displayName}
  ...
  alt Já existe
    ...
    API->>DB: INSERT INTO refresh_tokens (hash, expiresAt)
    API->>SES: sendEmailVerification(token)
    API-->>FE: 201 {accessToken, user} + Set-Cookie refreshToken
    FE->>FE: store accessToken in memory
    FE-->>U: Logado, com aviso "verifique seu e-mail"
  end`} />
...
<Mermaid chart={`sequenceDiagram
  ...
  alt User não existe ou sem passwordHash
    ...
    alt Inválido
      ...
      API-->>FE: 200 {accessToken, user} + Set-Cookie refreshToken
    end
  end`} />
...
<Mermaid chart={`sequenceDiagram
  ...
  API->>API: jwt.sign({mode:'login'}) (state)
  API-->>U: 302 Redirect → accounts.google.com (state)
  U->>G: Autoriza
  G-->>API: 302 Redirect /auth/google/callback?code&state
  API->>API: jwt.verify(state)
  API->>G: authorizationCodeGrant(code)
  G-->>API: tokens + claims
  API->>G: fetchUserInfo(access_token)
  ...
  API->>DB: INSERT INTO refresh_tokens
  API-->>FE: 302 Redirect /auth/callback?status=...&token=jwt + Set-Cookie refreshToken
...
<Mermaid chart={`sequenceDiagram
  ...
  alt Não encontrado ou expirado
    ...
    API->>DB: DELETE FROM refresh_tokens WHERE token_hash = ?
...
<Mermaid chart={`sequenceDiagram
  ...
  API->>DB: SELECT * FROM users WHERE email = ?
  alt Existe
    API->>API: gera token aleatório, sha256
    API->>DB: INSERT INTO password_reset_tokens (hash, expiresAt: +1h)
    API->>SES: sendPasswordResetEmail(rawToken)
  else Não existe
    Note over API: NÃO revela; sempre 200
  end
  API-->>FE: 200 {message: "Se o e-mail existir..."}
  U->>U: Recebe e-mail, clica no link
  U->>FE: Vai pra /auth/reset-password?token=...
  U->>FE: Digita nova senha
  FE->>API: POST /auth/reset-password {token, newPassword}
  API->>API: sha256(token)
  API->>DB: SELECT * FROM password_reset_tokens WHERE token_hash = ?
  alt Inválido / expirado / já usado
