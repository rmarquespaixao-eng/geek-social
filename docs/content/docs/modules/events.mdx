...
<Mermaid chart={`sequenceDiagram
  ...
  U->>FE: /roles/novo → preenche form, escolhe capa (jpg/png/webp ≤5MB)
  FE->>API: POST /events (multipart) {<br/>name, startsAt, durationMinutes,<br/>type=presencial, address, capacity?, cover}
  API->>API: Valida Zod (duration ∈ {60,120,...,600};<br/>endereço completo se presencial)
...
<Mermaid chart={`sequenceDiagram
  ...
  API->>DB: SELECT 1 WHERE tstzrange(starts_at, ends_at, '[)')<br/>&& tstzrange(:newStart, :newEnd, '[)') — conflito
  alt Vagas livres
...
<Mermaid chart={`sequenceDiagram
  ...
  alt Tem alguém na fila
    ...
    Note over API: Se startsAt < 48h, enviar reminder_48h imediato
  end
  API->>DB: COMMIT
  N->>WS: emit pro promovido<br/>("Sua vaga foi confirmada!")`} />
