...
<Mermaid chart={`sequenceDiagram
  ...
  FE->>API: POST /field-definitions {<br/>name, fieldType, selectOptions<br/>}
  API->>API: Valida via createFieldDefinitionSchema:<br/>- select exige ≥1 selectOptions<br/>- money exige selectOptions[0] = código ISO 4217
  API->>API: Gera field_key = slug(name)
  API->>DB: INSERT field_definitions<br/>(user_id, is_system=false, ...)
  API-->>FE: 201 { fieldDefinition }`} />
...
<Mermaid chart={`graph LR
  ...
  T --> Date
  T --> Boolean
  T --> Select[Select<br/>selectOptions: array de opções]
  T --> Money[Money<br/>selectOptions: 1 código ISO 4217<br/>BRL, USD, EUR...]`} />
