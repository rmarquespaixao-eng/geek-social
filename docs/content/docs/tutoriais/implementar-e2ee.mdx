...
description: Tutorial conceitual + prático pra implementar end-to-end encryption num chat — do zero até DMs, grupos, backup, rotação e auto-repair. Baseado na implementação real do Geek Social com Signal Protocol via libsignal-wasm e nas specs oficiais do Signal.
---

Este tutorial é o guia que eu queria ter tido quando comecei. Ele explica **o que cada conceito é**, **por que existe** e **como aplicar**, com referências ao código real do Geek Social e às specs oficiais do Signal. A página de [Criptografia E2EE](/docs/concepts/criptografia-e2ee) é a referência da arquitetura final — esta aqui é o caminho pra chegar lá.
...
A envia `CT` junto com a mensagem inicial; B recupera `SS` com `Kyber.Decaps(CT, sk_Kyber_B)`. Da perspectiva da integração com a libsignal, é a **mesma chamada** — `process_pre_key_bundle()` já junta tudo.
...
O Geek Social usa [@getmaapp/signal-wasm](https://github.com/getmaapp/signal-wasm) (fork de libsignal compilado pra WASM). Se for native, troque a camada do `SignalClient.ts` pelo binding nativo — o resto da arquitetura é igual.
...
- [X3DH — Extended Triple Diffie-Hellman](https://signal.org/docs/specifications/x3dh/) — handshake clássico
- [PQXDH — Post-Quantum Extended Diffie-Hellman](https://signal.org/docs/specifications/pqxdh/) — extensão pós-quântica
- [The Double Ratchet Algorithm](https://signal.org/docs/specifications/doubleratchet/) — gerenciamento de chaves de mensagens
- [Sesame](https://signal.org/docs/specifications/sesame/) — protocolo multi-device
- [PQXDH announcement](https://signal.org/blog/pqxdh/) — motivação e contexto histórico
- [Sealed Sender](https://signal.org/blog/sealed-sender/) — privacidade de metadados

### Implementações de referência

- [libsignal](https://github.com/signalapp/libsignal) — Rust, fonte canônica
- [@getmaapp/signal-wasm](https://github.com/getmaapp/signal-wasm) — fork WASM que o Geek Social usa

### Outras leituras

- [Whitepaper de criptografia do WhatsApp](https://www.whatsapp.com/security/) — descreve o uso de Sender Keys em produção
- [Signal Private Group System](https://signal.org/blog/signal-private-group-system/) — modelo pairwise pra grupos pequenos
- [Criptografia E2EE (referência do Geek Social)](/docs/concepts/criptografia-e2ee) — arquitetura final
