...
export default function Meta() {
  return (
    <div className="not-prose flex flex-wrap items-center gap-3 my-6 p-4 rounded-lg border bg-fd-card text-fd-card-foreground">
      <span className={`px-2.5 py-1 text-xs font-mono font-semibold rounded border ${METHOD_COLOR["DELETE"] ?? 'bg-fd-muted'}`}>
        DELETE
      </span>
      <code className="text-sm font-mono">/collections/{'{'}collectionId{'}'}/items/{'{'}itemId{'}'}</code>
