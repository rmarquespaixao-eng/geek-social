<!-- src/modules/offers/components/OfferDialog.vue
  Modal para criar oferta (compra ou troca) em um listing de outro usuário.
-->
<script setup lang="ts">
import { ref, computed, watch } from 'vue'
import { Tag, ArrowLeftRight, AlertTriangle, Image as ImageIcon, ChevronLeft } from 'lucide-vue-next'
...
import { createOffer, type OfferType } from '../services/offersService'

const props = defineProps<{
  open: boolean
  /** Listing alvo (do outro user). */
  listing: {
    id: string
    availability: 'sale' | 'trade' | 'both'
...
const canBuy   = computed(() => props.listing.availability === 'sale' || props.listing.availability === 'both')
const canTrade = computed(() => props.listing.availability === 'trade' || props.listing.availability === 'both')

const offerType = ref<OfferType | null>(null)
...
watch(() => props.open, (open) => {
  if (open) {
    offerType.value = canBuy.value && !canTrade.value ? 'buy'
                    : canTrade.value && !canBuy.value ? 'trade'
                    : null
    offeredPrice.value = props.listing.askingPrice ? String(props.listing.askingPrice) : ''
    message.value = ''
    error.value = null
    success.value = false
...
watch(offerType, async (t) => {
  if (t === 'trade' && myCollections.value.length === 0 && !collectionsLoading.value) {
...
const canSubmit = computed(() => {
  if (!offerType.value) return false
...
async function submit() {
  if (!offerType.value || !canSubmit.value || submitting.value) return
  ...
  try {
    ...
    error.value = code === 'LISTING_NOT_FOUND'                        ? 'Anúncio não encontrado.'
                : code === 'LISTING_NOT_ACTIVE'                       ? 'Este anúncio não está mais ativo.'
                : code === 'ITEM_NOT_AVAILABLE'                       ? 'Este item não está mais disponível.'
                : code === 'ITEM_NOT_FOR_SALE'                        ? 'Este item não está à venda.'
                : code === 'ITEM_NOT_FOR_TRADE'                       ? 'Este item não está aberto a trocas.'
                : code === 'CANNOT_OFFER_OWN_ITEM'                    ? 'Você não pode ofertar no seu próprio item.'
                : code === 'DUPLICATE_PENDING_OFFER'                  ? 'Você já tem uma oferta pendente para este item. Cancele a anterior antes de fazer uma nova.'
                : code === 'OFFERED_ITEM_NOT_OWNED'                   ? 'O item oferecido não é seu.'
                : code === 'OFFERED_COLLECTION_PRIVATE'               ? 'A coleção do item oferecido está privada.'
      ...
      <div class="p-5 space-y-4">
        <div class="flex items-start gap-3">
          <div class="flex h-10 w-10 items-center justify-center rounded-full bg-(--color-accent-amber)/10 flex-shrink-0">
            <component :is="canBuy ? Tag : ArrowLeftRight" :size="18" class="text-(--color-accent-amber)" />
          </div>
          <div class="flex-1 min-w-0">
            <h2 class="text-base font-semibold text-(--color-text-primary)">Fazer oferta</h2>
            <p class="text-xs text-(--color-text-muted) truncate">{{ itemName }}</p>
          </div>
        </div>

        <!-- Sucesso -->
        <div v-if="success" class="px-3 py-2.5 rounded-lg bg-(--color-status-online)/10 text-(--color-status-online) text-sm">
          ✓ Oferta enviada! O dono será notificado.
        </div>

        <template v-else>
          <!-- Tipo de oferta (só mostra seletor se ambos forem possíveis) -->
          <div v-if="canBuy && canTrade" class="grid grid-cols-2 gap-2">
            <button
              type="button"
              class="flex items-center justify-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition-colors"
              :class="offerType === 'buy'
            ...
            <button
              type="button"
              class="flex items-center justify-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition-colors"
              :class="offerType === 'trade'
                ? 'border-(--color-accent-amber) bg-(--color-accent-amber)/10 text-(--color-accent-amber)'
                : 'border-(--color-bg-elevated) text-(--color-text-secondary) hover:border-(--color-text-muted)/40'"
              @click="offerType = 'trade'"
            >
          ...
          <div v-if="offerType === 'buy'" class="space-y-3">
            <div>
              <label class="text-xs font-medium text-(--color-text-secondary)">Sua oferta (R$)</label>
              <input
              ...
              />
              <p v-if="listing.askingPrice" class="mt-1 text-[11px] text-(--color-text-muted)">
                Pedido pelo dono: R$ {{ Number(listing.askingPrice).toFixed(2).replace('.', ',') }}
              </p>
            </div>
          </div>

          <!-- Form de troca -->
          <div v-if="offerType === 'trade'" class="space-y-3">
            <!-- Step 1: pick collection -->
            <div v-if="!selectedCollectionId">
              <p class="text-xs font-medium text-(--color-text-secondary) mb-2">Escolha uma das suas coleções</p>
          ...
          <div v-if="offerType">
            ...
            <textarea
              ...
              rows="2"
              maxlength="2000"
              placeholder="Comente sua oferta..."
